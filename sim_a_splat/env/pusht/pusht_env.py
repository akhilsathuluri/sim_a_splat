...
from sim_a_splat.env.pusht.pymunk_override import DrawOptions
...
class PushTEnv(gym.Env):
    ...
    def __init__(
        ...
        self._seed = None
        self.seed()
        self.window_size_y = ws_y = 512  # The size of the PyGame window
        self.window_size_x = ws_x = 298  # The size of the PyGame window
        ...
        self.observation_space = spaces.Box(
            ...
            high=np.array([ws_x, ws_y, ws_x, ws_y, np.pi * 2], dtype=np.float64),
            shape=(5,),
            dtype=np.float64,
        )
        ...
        self.action_space = spaces.Box(
            ...
            high=np.array([ws_x, ws_y], dtype=np.float64),
            shape=(2,),
            dtype=np.float64,
        )
    ...
    def reset(self):
        seed = self._seed
        self._setup()
        # self.distractor.center_of_gravity = None
        ...
        if state is None:
            ...
            state = np.array(
                [
                    rs.randint(50, self.window_size_x - 50),  # agent pos x
                    rs.randint(50, self.window_size_y - 50),  # agent pos y
                    rs.randint(100, self.window_size_x - 100),  # block pos x
                    rs.randint(100, self.window_size_y - 100),  # block pos y
    ...
    def teleop_agent(self):
        ...
        def act(obs):
            ...
            if self.teleop or (mouse_position - self.agent.position).length < 10:
                self.teleop = True
                act = mouse_position
            return act
    ...
    def _setup(self):
        ...
        # Add walls.
        walls = [
            self._add_segment((5, self.window_size_y - 5), (5, 5), 2),
            self._add_segment((5, 5), (self.window_size_x - 5, 5), 2),
            self._add_segment(
                (self.window_size_x - 5, 5),
                (self.window_size_x - 5, self.window_size_y - 5),
                2,
            ),
            self._add_segment(
