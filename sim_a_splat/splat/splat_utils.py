...
from sim_a_splat.ellipsoids.mesh_utils import create_gs_mesh
from sim_a_splat.ellipsoids.covariance_utils import quaternion_to_rotation_matrix
from sim_a_splat.ellipsoids.covariance_utils import compute_cov
from sim_a_splat.ns_utils.nerfstudio_utils import GaussianSplat, SH2RGB
...
class GSplatLoader:
    ...
    def load_gsplat_from_nerfstudio(self, gsplat_location):
        ...
        self.opacities = torch.sigmoid(
            self.splat.pipeline.model.opacities.detach().clone()
        )
    ...
    def load_gsplat_from_json(self, gsplat_location):
        ...
        for key in keys:
            tensors[key] = torch.tensor(data[key]).to(
    ...
    def save_mesh(self, filepath, bounds=None, res=4):
        if bounds is not None:
            mask = torch.all(
                (self.means - bounds[:, 0] >= 0) & (bounds[:, 1] - self.means >= 0),
                dim=-1,
            )
        ...
        scene = create_gs_mesh(
            means.cpu().numpy(),
            quaternion_to_rotation_matrix(rots).cpu().numpy(),
            scales.cpu().numpy(),
            colors.cpu().numpy(),
            res=res,
            transform=None,
