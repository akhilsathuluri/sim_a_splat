...
from sim_a_splat.ellipsoids.covariance_utils import compute_cov
...
def _sh2rgb(sh):
    return np.clip(sh * _C0 + 0.5, 0.0, 1.0)
...
class PlySplatHandler:
    def __init__(
        ...
        self._load_saved_splats(path=f"{splat_assets_path}/{ply_name}")
        ...
        # rbt_model_instance is the Drake model instance index for the robot.
        # It is passed automatically from the env so it reflects env_objects on/off.
        self.rbt_idx = rbt_model_instance if rbt_model_instance is not None else 3
        self.blk_idx = 2
        self.rbt_drake_namespace = f"plant::{urdf_name.split('.')[0]}::"
        self.blk_drake_namespace = f"plant::{task_assets_name.split('.')[0]}::" if task_assets_name is not None else None
    ...
    def _load_saved_splats(self, path: str) -> None:
        device = torch.device(self.device)
        path_to_ply = Path(path).resolve()
        ...
        opacities = (1.0 / (1.0 + np.exp(-np.asarray(v["opacity"])))).reshape(-1, 1)
        scales = np.exp(
            np.stack(
                [
                    np.asarray(v["scale_0"]),
                    np.asarray(v["scale_1"]),
                    np.asarray(v["scale_2"]),
                ],
                axis=1,
            )
        )
        ...
        quats_t = torch.from_numpy(quats).float().to(device)
        covs = compute_cov(quats_t, scales_t).cpu().numpy()

        self.means = means
        self.covs = covs
        self.colors = colors
        self.opacities = opacities
        logging.info(f"Loaded splats from {path_to_ply}")
    ...
    def _add_robot_meshes(self, package_path=None, package_name=None, urdf_name=None):
        ...
        for ii in range(len(robot.links)):
            ...
            for visual in link.visuals:
                if visual.geometry.mesh:
                    ...
                    meshes.append(mesh)
                    color = visual.material.color if visual.material is not None else np.array([0.7, 0.7, 0.7, 1.0])
                    colors.append(color)
        ...
        self.link_name_to_mesh_idxs: dict = {}
        for idx, name in enumerate(self.mesh_link_names):
            self.link_name_to_mesh_idxs.setdefault(name, []).append(idx)

    def _add_task_meshes(self, task_assets_path=None, task_assets_name=None):
        self.task_mesh_frame_handle = None
        if task_assets_name is None:
            return
        task_mesh = o3d.io.read_triangle_mesh(task_assets_path + "/" + task_assets_name)
        mesh_color = np.tile(
    ...
    def draw_handler(self, msg: lcmt_viewer_draw):
        ...
        for idx in range(msg.num_links):
            if msg.robot_num[idx] == self.rbt_idx:
                ...
                # Strip "plant::" prefix to get the raw URDF link name, then look up
                # all mesh handle indices registered for that link.  Drake may have
                # bodies with no visual meshes (base, auxiliary frames) which simply
                # won't appear in the dict and are safely skipped.
                drake_link_name = msg.link_name[idx].replace("plant::", "", 1)
                for local_idx in self.link_name_to_mesh_idxs.get(drake_link_name, []):
                    ...
                    if local_idx < len(self.splat_links_handler):
            ...
            if msg.robot_num[idx] == self.blk_idx:
                ...
                if self.task_mesh_frame_handle is not None:
                    try:
                        local_msg = self.icp_transform.multiply(msg_transform_mesh)
