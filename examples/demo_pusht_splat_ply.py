...
def main(render_size, control_hz):
    ...
    eef_link_name = ["right_fr3v2_link8", "left_fr3v2_link8"]
    active_eef_index = 0  # right arm used for pushing
    num_dof = 18
    splat_assets_path = root / "assets/cppoc"
    match_object_name = "fr3_duo_drake"
    ply_name = "export_30000_cropped.ply"
    task_assets_path = (root / "assets/tblock_paper/").__str__()
    ...
    manipulator_eef_env = ManipulatorEEFWrapper(
        manipulator_env, eef_index=active_eef_index
    )
    camera_setup_info = {
        0: {
            "link_name": "world",
        ...
        1: {
            "link_name": eef_link_name[active_eef_index],
            "local_frame": tf.SE3(
                wxyz_xyz=np.concatenate(
    ...
    manipulator_eef_splat_env = PlySplatEnvWrapper(
        manipulator_eef_env,
        splat_assets_path=str(splat_assets_path),
    ...
    home_config = manipulator_eef_splat_env.get_initial_robot_pos()
    ...
    eef_ori = (
        RotationMatrix(
            Quaternion(info_manipulator["eef_infos"][active_eef_index]["eef_quat"])
        )
        .ToRollPitchYaw()
        .vector()
    )
